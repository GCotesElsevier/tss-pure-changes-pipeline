...
# A single transient read timeout used to blow up the whole batch via
# future.result() re-raising in the main thread — wasting every already-
# completed call (hit for real on Ajman's equivalent notebook, 2026-08-14,
# ~7600/7893 records in). Retried a few times with a short backoff before
# giving up for real; a persistent failure still raises, on purpose
# (silently dropping a record would be worse than failing loudly).
