...
# MAGIC Drops one or all of HBKU's per-scope resumptionToken control tables
# MAGIC (`SYNC_STATE_TABLES` in `config.py`) so the next `fetch_changes.py` run
# MAGIC starts that scope fresh from its own `DEFAULT_SINCE_DATES` entry
# MAGIC instead of resuming from wherever the last run left off.
# MAGIC
