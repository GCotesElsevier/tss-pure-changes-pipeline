...
# MAGIC Appends a "reset" row to one or all of HBKU's per-scope resumptionToken
# MAGIC control tables (`SYNC_STATE_TABLES` in `config.py`) pointing back at that
# MAGIC scope's own `DEFAULT_SINCE_DATES` entry, so the next `fetch_changes.py`
# MAGIC run resumes from there instead of from wherever the last run left off.
# MAGIC
